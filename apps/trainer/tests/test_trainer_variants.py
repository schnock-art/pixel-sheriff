...
def test_run_ptq_variant_quantizes_detection_split(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(variants_mod, "_onnx_session", lambda _path, cpu_only=False: FakeSession())
    monkeypatch.setattr(
        variants_mod,
        "preprocess_asset",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {"status": "ready", "mean_latency_ms": 12.5},
    )
...
def test_run_qat_variant_supports_detection_and_records_metrics(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class FakePipeline:
        def build_loaders(self, _job, _workdir):
    ...
    monkeypatch.setattr(
        variants_mod,
        "_write_variant_benchmark",
        lambda *args, **kwargs: {"status": "ready", "mean_latency_ms": 9.4},
    )
